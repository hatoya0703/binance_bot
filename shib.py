...
# ロジック用変数宣言
ticker = 'SHIB'          # 取引購入したい銘柄
currency = 'BUSD'       # 取引支払に使う銘柄
interval = 1*1         # チャート足（60*1 は 1分足）。while文を繰り返す間隔を秒数で指定
...
while True: #while文は条件式がtrueの間、実行され続ける(無限ループ)。Trueを直接渡すことで処理を永遠に繰り返させることができる。
    ...
    position = math.floor(position) # 売却ができる桁数まで切り捨て
    ...
    if position: # position（保有資産）がある時は購入、ないときは売却
        ...
        if df['price'].iloc[9] < df['-2sigma'].iloc[-1]:
            ...
            amount = math.floor(amount) # 購入ができる桁数まで切り捨て 
